...
CREATE VIEW video_list AS (
                          SELECT v.*,
                                 ...
                                 members.avatar_url,
                                 members.signature,
                                 COALESCE(statistics.forward, 0) AS forward,
                                 COALESCE(statistics.digg, 0)    AS digg,
                                 COALESCE(statistics.play, 0)    AS play,
                                 COALESCE(statistics.share, 0)   AS share,
                                 COALESCE(statistics.comment, 0) AS comment
